...
| `custodian_network_diagnostics` | No | Boolean (`true`/`false`) toggle to run Go DNS/TLS preflight probes for relevant AWS service endpoints before Custodian starts and log child process TCP socket snapshots while Custodian is running. DNS/TLS probe failures are logged as warnings and captured as execution errors. Concrete `aws_regions` are narrowed to regions with reachable resource service endpoints before Custodian runs; if none are reachable for a check, that check is skipped and later checks continue. Evidence from reachable checks is submitted before accumulated diagnostic errors are returned. If no concrete `aws_regions` are configured, service-derived probes are skipped unless `custodian_network_diagnostic_endpoints` is configured. For AWS resource types not mapped to diagnostic services, diagnostics warn and continue unless explicit endpoints are configured. Invalid configured endpoints still fail before execution. Default: `false`. |
